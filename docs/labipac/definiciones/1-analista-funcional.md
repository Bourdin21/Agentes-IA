...
- **Modo de precio por Practica: Por Unidad vs Manual (sesion 7, 2026-08-24):** el cliente reporto que no todas las Practicas encajan en el modelo de "unidades bioquimicas" (ej. `LIBRETA SANITARIA`, un tramite administrativo sin relacion real con analisis bioquimico) — pidio poder ponerle un precio a mano, o "unidades con decimales" para forzar el precio deseado. Se identifico que la segunda opcion no resuelve el problema de fondo (el precio se sigue recalculando cada vez que cambia el `PrecioPorUnidad`, igual que el riesgo RA-17 ya conocido). Se confirmo con el cliente: cada Practica declara un `ModoPrecio` (`PorUnidad` | `Manual`), elegible libremente en cualquier Practica (no solo administrativas). En modo `Manual`, el precio de la Practica se carga a mano y **tambien varia por Centro de Salud** (mismo criterio que el modo `PorUnidad`, pero como valor libre en vez de una cantidad de unidades × una tasa). Un Perfil solo puede componerse de Practicas en modo `PorUnidad` (nueva regla, evita mezclar un precio fijo dentro de la suma de unidades que determina `Unidad` del Perfil).

## Historial de ajustes
- 2026-08-24 (sesion 7): Modo de precio por Practica (`PorUnidad`/`Manual`). El cliente senalo que no toda Practica es "por unidades bioquimicas" (caso LIBRETA SANITARIA, tramite administrativo). Se descarto la idea de `CantidadUnidades` con decimales (no resuelve el problema de fondo, el precio se sigue recalculando). Se confirmo con el cliente: interruptor libre por Practica, precio Manual varia por Centro de Salud (valor libre, no formula), Perfiles solo pueden componerse de Practicas en modo `PorUnidad`. Requiere: `UnidadBioquimica.ModoPrecio` (enum nuevo), entidad nueva `PrecioManualUnidadBioquimicaCentroSalud` (Practica × Centro, solo para items en modo Manual — no una matriz completa), ajuste de `GetPrecioVigenteAsync` y de la validacion de composicion del Perfil.
