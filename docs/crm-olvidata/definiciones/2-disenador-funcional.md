...
- 2026-07-24: Diseño exprés del ajuste de UI en `Notifications/Index.cshtml`. Propuesta: (1) ícono `fas fa-xmark` en lugar de `fas fa-trash` en el botón de eliminar individual y en el botón de eliminar en lote ("Eliminar leídas"), mismo estilo `btn btn-sm btn-outline-danger` ya usado (sin cambiar color ni tamaño, solo el glifo); (2) los 2 `onsubmit="return confirm(...)"` actuales pasan a `Swal.fire({icon:'warning', title, text, showCancelButton:true, confirmButtonText:'Sí, eliminar', cancelButtonText:'Cancelar', confirmButtonColor:'#ef4444'}).then(...)` — mismo patrón textual usado en `Campanas/Index.cshtml` (confirmación de borrado de campaña) — y solo si `isConfirmed` se dispara el `submit()` del form (los forms dejan de tener `onsubmit`, pasan a manejarse por JS con `preventDefault` + confirmación async, igual que el resto de las pantallas del proyecto). No hay historias de usuario nuevas (ajusta la UI de HU ya cerrada, sin cambiar comportamiento funcional del borrado). Condición de paso a Arquitectura: cumplida.
