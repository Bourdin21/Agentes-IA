...
- `Resumen.cshtml`: eliminado el overlay bloqueante + `beforeunload` (ya no aplica, el usuario navega libre). Polling de 5s extendido para cubrir tambien `Aplicando` (antes solo `Procesando`).
- Verificado end-to-end con Playwright headless contra dev real: click en Aplicar responde en ~1s (antes bloqueaba hasta terminar todo el volcado), navegacion libre confirmada mientras corre en background, notificacion "Precios actualizados" confirmada en la campanita al terminar. Build limpio, 43/43 tests en verde.
- Detalle completo en `trazabilidad.md`. Sin commit/push.

### 2026-08-31 (mismo dia, tercera tarea) — Boton "Actualizar precios" reemplaza pantallas de importacion manual (Importar/Historial eliminadas)
- `Proveedor` gana `UrlListaVenta`/`UrlListaCompra` persistentes (antes efimeras, solo por-importacion). Migracion `AddUrlsListaPreciosProveedor` (aditiva) + script SQL idempotente en `docs/vino-y-se-fue/scripts/`. Nueva seccion en `Proveedor/Index` para cargar/renovar esos links (se siguen venciendo semanalmente segun el proveedor, ahi es donde se actualizan).
- Eliminadas pantallas `Catalogo/Importar` + `Catalogo/Historial` (controller, vistas, links de sidebar). Nueva accion `CatalogoController.ActualizarPrecios` (POST admin) que lee las URLs del Proveedor y reutiliza el mismo flujo de background + Resumen/Aplicar/Cancelar ya existente sin tocarlo — el gate humano de revisar el diff se mantiene. Boton "Actualizar precios" en `Productos/Index` (solo admin) con preconfirmacion via el patron `.btn-swal-confirm` ya usado en 10 pantallas del proyecto (sin modal custom nuevo). Codigo muerto removido: `GetHistorialAsync` + 4 ViewModels sin consumidores.
- Verificado end-to-end con Playwright headless contra dev real: rutas viejas dan 404, campos nuevos en Proveedor persisten, boton dispara confirmacion y el flujo completo (Procesando→Preview→Cancelar) funciona con datos reales del proveedor (311/232/1187/0). Build limpio, 43/43 tests en verde. Import de prueba cancelado (sin tocar datos reales de dev).
- Detalle completo en `trazabilidad.md`. Sin commit/push, migracion no aplicada en produccion.

### 2026-08-31 (mismo dia, tarea siguiente) — Catalogo de Productos dinamico (AJAX/JSON) + Data Protection persistente
- **Catalogo dinamico:** `ProductosController.BuscarJson` (nueva accion GET, reutiliza `IProductoCatalogoService.GetCatalogoAsync` sin tocarlo) + reescritura del `@section Scripts` de `Productos/Index.cshtml` para que busqueda/filtros/paginacion/limpiar disparen `fetch()` en vez de `form.submit()`. Primer render sigue siendo SSR; interacciones posteriores son AJAX con `history.pushState` (URL bookmarkable, funciona con atras/adelante). JS reconstruye tabla/paginacion/chips/contador/URLs de exportacion replicando el markup Razor exacto (grupos por bodega, columna Costo solo-admin, columna batch-cart cuando hay pedido borrador). Corregido de paso un escape de HTML faltante en `renderCart` (pre-existente, no introducido ahora).
- Verificado con Playwright headless contra dev real (datos de `VinoSeFue_dev`, login SuperUsuario semilla): busqueda, filtro de categoria, paginacion y limpiar funcionan sin recarga completa, URL sincronizada, 0 errores de consola. Carrito por lote (pedidoId) no probado en navegador por falta de pedido Borrador en dev — logica original sin cambios, solo reinvocada.
- **Data Protection persistente:** mismo fix que marihogar/eleven-la-plata/virtualwallet — `Program.cs` con `AddDataProtection().PersistKeysToFileSystem(keys/)`, patron calcado de `Eleven.Web/Program.cs`. Perfil Web Deploy con skip-rules explicitas + `.gitignore` actualizado. Verificado: se crea `keys/key-*.xml` real al levantar el server de dev.
- Build limpio, 43/43 tests en verde (sin tests nuevos para esta tarea — cambio de UI/infra, no de logica testeable unitariamente). Sin migracion EF, sin cambios de Domain/DB.
- Detalle completo en `trazabilidad.md` (entrada 2026-08-31, segunda tarea del dia). Sin commit/push (pendiente revision del cliente).

### 2026-08-31 — Fix: importacion de listas de precios rota por cambio de plataforma del proveedor (milistadeprecios.com HTML -> soysowi.com SPA+JSON)
- Cliente reporto que las URLs nuevas de las listas L7/L5 (proveedor AQV) no importaban. Causa raiz: el proveedor migro de HTML estatico (tabla columnar, formato que `HtmlPriceListParser` sabe leer) a una SPA Vue en `soysowi.com` que renderiza todo client-side — el HTML descargado por `HttpClient.GetStringAsync` es un shell vacio (`<div id="app"></div>`), sin tabla que parsear.
- Investigacion: descargados y leidos los bundles JS de la SPA (`index-*.js`, `CatalogoPublicoView-*.js`, `catalog-*.js`) para ubicar el endpoint real que consume el frontend: `GET {origin}/api/products/catalog/public/{listSlug}/{token}/` (o sin `listSlug` si el link tiene un solo segmento de path), que devuelve JSON `{ estado, bodegas: [{ bodega, productos: [{ codigo, descripcion, unidades_por_presentacion, precio }] }] }`.
- Fix: nueva clase `SowiCatalogParser` (Infrastructure) con `TryBuildApiUrl` (regex sobre el path publico -> URL de API JSON equivalente) y `Parse(json)` (deserializa con `JsonNamingPolicy.SnakeCaseLower`, mapea a `CatalogoFilaParseada` reutilizando el mismo DTO que ya usaba el parser HTML). `CatalogoImportacionService` ahora prueba primero si la URL matchea el formato Sowi nuevo antes de caer al parser HTML legacy — **coexistencia**, no reemplazo, por si otro proveedor sigue usando el formato viejo. Sin cambios de Domain/DB/UI.
- Verificado end-to-end contra las 2 URLs reales del cliente (no solo mockeado): L7 y L5 parsearon 1498 productos cada una, agrupados por bodega, con precios distintos entre listas (esperado). 9 tests unitarios nuevos + 43/43 tests del proyecto en verde. Build limpio.
- Detalle completo en `trazabilidad.md` (entrada 2026-08-31). Sin commit/push (pendiente revision del cliente).
