...
- **Negocio (Services)**: `VentaWorkflowService`, `UnidadMedidaConversionService`, `RecargoCuotasService`, `ListaPreciosProveedorImportService`, `CatalogoMigracionService`, `CuentaCorrienteEmpleadoService`, `CajaService` (diaria+mensual), `AfipFacturacionService` (incluye emisión de NC/ND), `AnulacionVentaService`, `DevolucionMercaderiaService`, `EntregaMarkupService`, `AumentoMasivoService`, `DashboardService` (3 niveles: día / salud financiera / tendencias), `AjusteStockService` (corrección manual auditada + venta con stock negativo permitido).
