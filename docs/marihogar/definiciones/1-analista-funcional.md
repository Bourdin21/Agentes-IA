...
1. **Precio en negro** (`Producto.PrecioEfectivo`) — aplica **únicamente** pagando en efectivo y **sin pedir factura**.
2. **Precio con IVA / recargo 21%** (`Producto.PrecioLista` = `PrecioEfectivo` + 21%, `PorcentajeHelper.AplicarRecargo`) — aplica en **cualquier otro caso**: efectivo con factura (aunque a veces no se llegue a emitir la factura real, el precio cobrado es igual el "con IVA" — es una política de precio, no está atada a que exista un `ComprobanteAfip` real), transferencia, MercadoPago, Banco Carrefour, o tarjeta de crédito (tarjeta SIEMPRE cobra este precio, sin excepción).

**Decisión confirmada con el cliente** (`AskUserQuestion`, ya que el pedido original no mencionaba dónde caían Transferencia/MercadoPago/Banco Carrefour): esos 3 métodos van siempre con el precio "con IVA", igual que efectivo-con-factura — quedan afuera del precio en negro exclusivamente el efectivo sin factura.

Mecánica ya existente desde CR-38 (`VentaItem.TipoPrecio`, enum `Contado`/`Tarjeta`) sigue siendo la implementación correcta de esto — no fue necesario tocar el modelo de datos, solo la etiqueta visible («Negro (efvo. s/factura)» / «Con IVA (21%)» en `Ventas/Create.cshtml`/`Edit.cshtml`/`Details.cshtml`) y documentar la regla acá. El selector por línea (radio, habilitado para Administrador y Vendedor) es la forma en que se fija qué precio corresponde a esa línea.
