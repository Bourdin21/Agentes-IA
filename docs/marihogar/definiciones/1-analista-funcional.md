...
1. **Precio efectivo** (`Producto.PrecioEfectivo`) — aplica **únicamente** pagando en efectivo.
2. **Precio de lista / recargo 21%** (`Producto.PrecioLista` = `PrecioEfectivo` + 21%, `PorcentajeHelper.AplicarRecargo`) — aplica en **cualquier otro caso**: efectivo con factura, transferencia, MercadoPago, Banco Carrefour, o tarjeta de crédito (tarjeta SIEMPRE cobra este precio, sin excepción).

**Decisión confirmada con el cliente** (`AskUserQuestion`, ya que el pedido original no mencionaba dónde caían Transferencia/MercadoPago/Banco Carrefour): esos 3 métodos van siempre con el precio "de lista", igual que efectivo-con-factura — queda afuera del precio efectivo exclusivamente el efectivo sin factura.

Mecánica ya existente desde CR-38 (`VentaItem.TipoPrecio`, enum `Contado`/`Tarjeta`) sigue siendo la implementación correcta de esto — no fue necesario tocar el modelo de datos, solo la etiqueta visible («Precio efectivo» / «Precio de lista (+21%)» en `Ventas/Create.cshtml`/`Edit.cshtml`/`Details.cshtml`) y documentar la regla acá. El selector por línea (radio, habilitado para Administrador y Vendedor) es la forma en que se fija qué precio corresponde a esa línea.
