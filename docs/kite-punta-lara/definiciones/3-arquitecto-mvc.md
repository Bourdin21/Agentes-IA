...
**Presentacion (`public/` + `src/Controllers/` + `templates/`)**
- `public/index.php`: front controller unico, tabla de rutas simple (array `ruta => [Controller, accion]`), sin framework completo (Slim/Laravel) — alcance de 7 pantallas no lo justifica, evita una dependencia y una capa de abstraccion que no se va a aprovechar.
- `src/Controllers/`: `HomeController` (P1/P4 — detecta si hay sesion activa para elegir tabla de equipo default vs. personalizada), `AuthController` (P2/P3 — login/registro), `PerfilController` (P5), `BitacoraController` (P6/P7). Los controllers solo arman el ViewModel (array/DTO) y lo pasan a la plantilla — nunca contienen las reglas de `MotorDeCalculoService`.
- `templates/`: PHP plano (no Twig/Blade, para no sumar dependencia en un proyecto de este tamaño), con `htmlspecialchars()` consistente en toda salida de datos de usuario (bitacora social muestra comentarios cargados por otros usuarios — superficie de XSS real si no se escapa).
- Sesion nativa de PHP (`session_start()`) para login; helper `Csrf::token()/Csrf::validate()` centralizado, usado en los 4 formularios POST (login, registro, guardar equipo, cargar sesion) — no reinventar la proteccion CSRF por pantalla.
