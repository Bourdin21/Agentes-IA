...
- 2026-09-10 (implementador-astro-front, repaso con las reglas nuevas del rol - estudio de antigravity.google): Se midieron las 23 reglas AG/AG-N con Chrome real, no leyendo codigo. Aplicadas: red de seguridad global de `prefers-reduced-motion` (el punto de "en vivo" latia en loop con `reduce`); anillo de foco global medido >=3:1 entre enfocado y sin enfocar (7 elementos no llegaban; `--accent` sobre blanco da 2,98:1); el tooltip de hora se reubica en vez de cerrarse ante el scroll que provoca el propio foco (con teclado no se veia nunca, tambien en produccion); fuera `<ClientRouter />` en un sitio de una sola pagina (5,6 KB gz, 11% del JS; los scripts arrancan en `DOMContentLoaded`); boton Compartir oculto sin JS; hex de estilos de componentes a tokens; transicion base 200 ms. Encontrado al medir, fuera de la lista: `[hidden]` perdia contra un `display` de autor (ahora `[hidden] { display: none !important }`), la capa SVG de horas era tabulable y sin nombre porque Chrome vuelve enfocable a todo SVG con listeners de foco (`tabindex=-1` en la capa), `transition: all` animaba el outline, y el HTML servido llevaba 44 comentarios internos (19%; pasaron a `{/* */}`). N6 NO aplicable: el hosting ignora `Header` y `Expires` en `.htaccess` (probado con tres variantes en un directorio descartable). Commit `cff08d1`.
- 2026-09-10 (pronostico de respaldo del pilote, pedido de Joaquin): "Ahora en el rio" muestra el pronostico de la hora actual, rotulado como tal, cuando el Pilote Norden no responde o su lectura tiene mas de 2 h. Umbral medido: CARP publica con 15 a 75 min de atraso andando normal. Se busca el pilote por nombre: el SMN no lo reemplaza (aeropuerto tierra adentro). De noche queda el rango del dia. Commit `aa8739c`.
- 2026-09-10 (render 3D descartado otra vez): se retomo por error la rama 3D al leer "refactoreo web a WebGL"; Joaquin ya lo habia descartado. Revertido todo: preview `/webgl/` borrado del servidor, contrato `sectoresOffshore` revertido (`07d009d`). Produccion nunca mostro la escena. Las ramas `render-3d` y `webgl` quedan en el remoto, sin mergear.
- 2026-09-10 (segundo repaso, con el rol completo): stack, fondos, reveal, verificacion, deploy y cierre, no solo la lista AG. `tilt.ts` se cargaba sin un solo `[data-tilt]`/`[data-magnetic]` en el sitio: fuera (N10). `scripts/deploy.sh` parametrizable por archivo de credenciales (`bash scripts/deploy.sh .env.deploy.otro`). Astro 7.2.10 -> 7.3.2 en commit propio (`dece62d`). Smoke test de clicks reales: los 17 controles visibles reciben el puntero, cada accion probada con su efecto, cero errores de consola y cero requests fallidos, escritorio y mobile. Lighthouse en produccion: mobile 99/100/100/100 (TBT 30 ms, LCP 2,0 s, 171 KB), escritorio 99/100/100/100. Patron nuevo en el catalogo: PAT-026 (scroll horizontal que se nota). Commit `60411bd`. PENDIENTE DE DECISION: AG-02 completo (tokens de dos niveles; hoy 53 hex + 81 rgba distintos en `global.css` y 69 rgba en componentes) y la variante WebGL/GPGPU del fondo, que el rol reserva para cuando se pide explicitamente. TRAMPA DE MEDICION: despues de un upgrade de Astro, el dev server servia `504 Outdated Optimize Dep` para gsap y el dashboard no arrancaba; borrar `node_modules/.vite` y levantar con `--force` antes de probar.
- 2026-09-12 (bug de direccion: el navegable pasa a ser un arco de grados): Joaquin reporto que un dia que no era de kite se informaba como si lo fuera. Con viento del sur todo el dia, 13 de 14 horas quedaban bloqueadas y la de las 19:00, con 156 grados, se colaba con 16,8 nudos: alcanzaba una hora para dar el dia por navegable. La causa era el borde: la regla estaba escrita como lista de sectores de 45 grados (S, SO, O = 157,5 a 292,5) y el limite real parte al SE justo en 135, o sea en el medio de un sector. Regla nueva, textual de Joaquin: "el rango navegable va desde NO 315 hasta SE 135, todos los valores comprendidos en este rango entran (350, 360, 0, 50, 100, etc)" — precision de lo que habia dicho el 2026-09-02 ("SE, E, NE, N, NO son vientos navegables"), no un cambio de opinion. Se agrego Direccion::enArco (bordes incluidos, contempla que el arco cruza el norte), el bloqueo pasa a ser el complemento del arco, el terral usa el mismo arco, y el sector arrachado pasa a ser el arco ESE 112,5 a SE 135 con el aviso nuevo de "puede no bornear" cuando el viento se queda ahi 3 horas seguidas sin rotar (umbral en config, PENDIENTE de confirmar con Joaquin). CONSECUENCIA: dos vientos que la pantalla muestra como "SE" ahora se deciden distinto (130 entra, 140 bloquea); por eso la direccion se muestra siempre con los grados al lado, y el comentario que justificaba usar sectores se reescribio. Verificado con 33 casos de borde, 9 de la señal de arrachado y el dato real del dia: 14 de 14 horas bloqueadas en produccion. Commit 02219ad.
