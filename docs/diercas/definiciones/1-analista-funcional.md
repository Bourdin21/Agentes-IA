...
### Contenido real de negocio confirmado por el cliente (2026-08-13, presupuesto aprobado — inicio de implementación)

**Reemplaza la discrepancia declarada en v3.** El cliente no confirmó ninguna de las dos opciones planteadas (mantener 5 líneas del sitio viejo, o quedarse con las 3 del dossier) — trajo una **tercera formulación propia**, en sus palabras, con 3 ramas de negocio:

1. **Infraestructura** — servicios de infraestructura informática: data centers, servidores, redes LAN, fibra óptica. Tienen equipo propio (OTDR y Fusionadora) al servicio del cliente — dato técnico de peso para la página de Servicios (credibilidad de "tienen el equipo, no subcontratan").
2. **Provisión de equipos y accesorios e insumos informáticos** — para organismos públicos y privados. **Nota**: esto es, en sustancia, la misma línea "venta de equipos e insumos" del sitio viejo que la v3 daba por posiblemente reemplazada — el cliente la confirma como vigente, con otro nombre.
3. **Infraestructura de eventos** — configuración de todos los dispositivos para conferencias, charlas y eventos similares. **Línea nueva, no estaba ni en el sitio viejo ni en el dossier.**

**Resuelto 2026-08-13 (gate de Joaquín)**: Ciberseguridad y Audio/Video quedan **fuera de la Etapa 1** — se documentan como posibles implementaciones de Etapa 2, sin comprometerse todavía. La página de Servicios de Etapa 1 se maqueta solo con las 3 ramas confirmadas (Infraestructura, Provisión de equipos, Eventos), pero el layout queda preparado para sumar bloques 4/5 sin rediseño si Etapa 2 los confirma.

**Clientes — confirmado 2026-08-13**: se mantiene el mismo criterio ya usado por Diercas (logos con listado existente) + se agregan los clientes nuevos aportados, **agrupados por rubro**. Lista aportada por el cliente:
- Privados: EDELP, Club Estudiantes de La Plata, LPRC (La Plata Rugby Club), Colegio Patris, Aloise (ya estaba en la v2), CAAITBA, Bodegón Urquiza, Flora Café, Lo de Edgardo.
- Públicos: Ministerio de Salud de la Provincia (ya estaba en v2), Poder Judicial de la Provincia (ya estaba en v2), Presidencia de la Nación (nuevo — de mayor peso institucional que todo lo relevado hasta ahora), Hospitales (sin especificar cuáles).

**Certificaciones/credenciales de confianza — nuevo (2026-08-13)**:
- **RITE** (Registro de Integridad y Transparencia para Empresas y Entidades) — registro oficial y gratuito de la Oficina Anticorrupción (`rite.gob.ar`), verificado por research: acredita el Programa de Integridad de la empresa y es válido para cumplir el requisito de Programa de Integridad en licitaciones públicas donde es obligatorio. Es un dato real y verificable, no una etiqueta de marketing — vale la pena enlazar al perfil público de Diercas en RITE si lo tienen, no solo mostrar el nombre.
- **QR Data Fiscal** (ARCA/AFIP, Formulario 960/D) — verificado por research: es un QR oficial que ARCA emite para que el negocio lo publique en su sitio, permite a cualquier visitante consultar el comportamiento fiscal del comercio. **Es un asset que el cliente debe generar desde su propio portal de ARCA** (no lo genera ni lo diseña el estudio). **Resuelto 2026-08-13: pasa a Etapa 2** (junto con el logo vectorial, que ya estaba en Etapa 2) — la franja de Certificaciones de Etapa 1 se lanza solo con RITE + texto de aptitud para contratar con el Estado, sin el QR todavía.
- Diercas declara cumplir todos los requisitos para contratar con el Estado (Nación, Provincia, Municipio) — se muestra como texto de respaldo institucional, reforzado por RITE + Data Fiscal como evidencia concreta, no como afirmación sin sustento.

**Trabajos realizados — confirmado que sigue en Etapa 2**: el cliente todavía está formulando el contenido (fotos + descripción) — no hay insumos todavía, se mantiene tal cual estaba planeado en v1/v2 (sección aparte, cargada por Joaquín cada 6 meses), simplemente sin fecha de inicio de contenido todavía.

**Contacto — confirmado y ampliado (2026-08-13)**: mail `administracion@diercas.com.ar`, WhatsApp `221 570 6954`. El formulario de contacto (ya definido en Diseño v1) debe enviar el mensaje **desde** `no-reply@olvidata.com.ar` **hacia** `administracion@diercas.com.ar` — mismo patrón de remitente que usa el estudio en otros proyectos (remitente propio del estudio, destinatario el cliente), no un envío en nombre del visitante.

**Decisión de stack/hosting — confirmada, ya no es un supuesto (v3 lo dejaba como "candidato natural")**: Astro, hosteado en **DonWeb** (no queda abierto), partiendo de la base técnica real de `C:\Sistemas\labipac-front` (mismo patrón de `Layout.astro`, mismo endpoint PHP `/api/contact.php`) — con la instrucción explícita de aplicar mejoras de UX/UI por sobre la base, no clonarla literalmente.
...
- 2026-08-13 (v4 — presupuesto aprobado, arranca implementación): cliente aprobó el presupuesto. Trajo contenido real de negocio: 3 ramas propias (Infraestructura con OTDR/Fusionadora propios, Provisión de equipos/insumos, Infraestructura de eventos — esta última nueva), lista ampliada de clientes públicos/privados agrupados por rubro (suma Presidencia de la Nación, EDELP, LPRC, Colegio Patris, CAAITBA, Bodegón Urquiza, Flora Café, Lo de Edgardo, Hospitales), certificaciones RITE y QR Data Fiscal (ambas verificadas por research — ver arriba), confirmación de stack (Astro) y hosting (DonWeb) ya no como supuesto sino decidido, y destino exacto del formulario de contacto (no-reply@olvidata.com.ar → administracion@diercas.com.ar). **La discrepancia de v3 no quedó resuelta, cambió de forma**: ahora es "¿dónde quedan Ciberseguridad y Audio/Video?", ninguna de las dos mencionada en las 3 ramas que el cliente sí confirmó — pregunta abierta nueva, no asumida en ningún sentido.
- 2026-08-13 (v5 — gate de Implementación): Joaquín resolvió el bloqueo de Servicios: Ciberseguridad y Audio/Video quedan como posibles implementaciones de Etapa 2 (no comprometidas), QR Data Fiscal también pasa a Etapa 2. Etapa 1 arranca con el resto del alcance ya definido, sin cambio de precio (la reasignación no agrega ni quita módulos del WBS ya aprobado, solo reordena contenido dentro de Servicios/Certificaciones). Repositorio creado en `C:\Sistemas\diercas-front`, workspace vinculado a `Agentes-IA`. Deploy pendiente de credenciales FTP que Joaquín va a proveer después — no bloquea el desarrollo local.
