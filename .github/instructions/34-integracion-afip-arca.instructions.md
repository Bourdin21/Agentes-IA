...
**Advertencia sobre proxies WSDL viejos como fuente de verdad**: al verificar el orden de un campo nuevo contra una copia local de un WSDL, **re-verificar siempre contra el WSDL de PRODUCCION real de AFIP en el momento** (o contra un proyecto que ya factura de verdad hoy), nunca confiar ciegamente en una copia de otro proyecto sin fecha de verificacion reciente.

**Actualizacion 2026-09-07 — gap de `CondicionIVAReceptorId` en delicias-naturales, CORREGIDO**: ARCA notifico formalmente (carta fechada 07/09/2026) que desde el **01/12/2026** rechaza automaticamente cualquier comprobante sin este campo. El `Reference.cs` de delicias-naturales (`Web References\ws_factura_afip\Reference.cs`) es un proxy ASMX viejo generado con `wsdl.exe`, **sin ningun `XmlElementAttribute` explicito en toda la clase** — la serializacion depende 100% del orden FISICO de declaracion de las propiedades en el codigo, no de un atributo `Order`. Para agregar un campo nuevo a este tipo de proxy: insertar el campo privado + la propiedad publica a mano, en el archivo generado, en la posicion exacta que corresponde segun el orden real de AFIP (documentado arriba) — nunca en un partial class aparte (el orden de compilacion entre archivos partial no esta garantizado). Verificacion sin tocar la red de AFIP: cargar el DLL compilado y serializar una instancia de prueba con `System.Xml.Serialization.XmlSerializer` para confirmar visualmente que el campo nuevo aparece en la posicion XML correcta — mas rapido y sin riesgo que armar un round-trip real contra homologacion, y suficiente para descartar un rechazo por orden de campos incorrecto. Fix real en `Helper/FacturacionAfipService.cs` (metodo `ObtenerCondicionIVAReceptorId`): mapea la condicion IVA del cliente a la tabla oficial de AFIP (Responsable Inscripto=1, IVA Exento=4, Consumidor Final=5 default para clientes sin condicion asignada, Monotributo=6). **Cualquier otro proyecto del estudio con un proxy SOAP viejo (no un `AfipService.cs` moderno tipo marihogar) debe auditarse por el mismo gap** — buscar si `CondicionIVAReceptorId` existe en su proxy antes del 01/12/2026.

**Patron de rollout togglable para un campo nuevo obligatorio-a-futuro**: a pedido explicito del cliente, el envio de `CondicionIVAReceptorId` en delicias-naturales quedo detras de un AppSetting (`AfipEnviarCondicionIVAReceptor`, default `false`) en vez de mandarse siempre desde que se corrigio el codigo — mientras el campo no sea realmente obligatorio, preferir no exponer un comportamiento nuevo en produccion mas tiempo del necesario, aunque sea de bajo riesgo. Para un campo de tipo valor (`int`, no nullable) en un proxy ASMX viejo sin `XmlElementAttribute`, la forma correcta de hacerlo condicionalmente ausente del XML es el patron **`<Campo>Specified`** que `XmlSerializer` reconoce automaticamente: agregar una propiedad `public bool <Campo>Specified` (con `[XmlIgnoreAttribute()]`) junto al campo — si es `false`, el elemento no se serializa en absoluto; si es `true`, se serializa con el valor asignado. Se verifico offline (cargando el DLL compilado y serializando una instancia de prueba con `XmlSerializer`, sin tocar la red de AFIP) que con el flag apagado el XML queda bit a bit igual que antes del cambio, y con el flag prendido el campo aparece en la posicion correcta. Activarlo mas adelante es meramente un cambio de `Web.config` (sin tocar codigo ni recompilar), asi que no hay excusa para no cambiarlo con tiempo antes del 01/12/2026.
