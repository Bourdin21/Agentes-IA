...
- **Keyring de Data Protection efimero en hosting compartido invalida sesion/antiforgery en cada deploy o reciclado de pool**: en hosting compartido (SmarterASP/site4now) el Application Pool no tiene un profile de usuario cargado, asi que `AddDataProtection()` sin configurar cae a un keyring en memoria — se pierde en cada reciclado de pool y en cada deploy de codigo, invalidando de golpe la cookie de Identity y los antiforgery token de todos los usuarios con sesion activa (mismo sintoma de fondo, "sin profile de usuario cargado", que motivo `X509KeyStorageFlags.MachineKeySet` en la integracion AFIP — ver `34-integracion-afip-arca.instructions.md`). Confirmado y resuelto en produccion real en marihogar, virtualwallet y La Platense. Requiere DOS partes, no solo una:
  1. En `Program.cs`, antes de `AddIdentity`:
     ```csharp
     builder.Services.AddDataProtection()
         .PersistKeysToFileSystem(new DirectoryInfo(Path.Combine(builder.Environment.ContentRootPath, "DataProtection-Keys")))
         .SetApplicationName("<NombreDelProyecto>");
     ```
     Sin `ProtectKeysWithX`: no hay DPAPI de maquina confiable en este hosting compartido, las claves quedan sin cifrar en disco — mismo nivel de exposicion que `appsettings.Production.json`, que ya vive sin cifrar en el mismo servidor.
  2. Excluir la carpeta `DataProtection-Keys/` tanto de git (`.gitignore`) como del sync de Web Deploy (`-skip:objectName=dirPath,absolutePath=DataProtection-Keys` en el comando/script de `msdeploy`) — sin esto, el primer deploy siguiente vuelve a borrar el keyring recien creado y el fix no sirve de nada (la carpeta la crea la app sola la primera vez que corre, no hace falta crearla a mano).
  Aplicar desde el arranque (`Program.cs`) de todo proyecto nuevo del estudio que se vaya a deployar a hosting compartido tipo SmarterASP/site4now — no esperar a que el sintoma ("se desloguea en cada deploy") aparezca en produccion.
