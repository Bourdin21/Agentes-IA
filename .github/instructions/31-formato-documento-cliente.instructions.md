...
4. **Seccion de inversion/pricing** (nombre segun el tipo de oferta):
   ...
   - Desarrollo a medida por horas (la mayoria de los proyectos del estudio): mantener las tablas ya definidas en `27-presupuesto-parametros.instructions.md` — `Etapa 1 (MVP)`, `Etapa 2`, `Total del proyecto` (con linea Tokens IA explicita al 25% del total, ver regla vigente), y `Mantenimiento anual` como tabla separada. Se envuelven con el mismo estilo visual (headers, italics de fine print) pero el contenido numerico no cambia por este formato.
