...
- **ScrollSmoother (scroll con inercia) es alto riesgo — confirmar explicitamente antes de implementarlo**, nunca agregarlo por iniciativa propia como "mejora". Rompe `position: sticky` y `scrollIntoView()` nativo dentro del contenido suavizado, exige mover navbar/botones fijos fuera del wrapper. Ver seccion dedicada en el rol para los fixes exactos — y estar listo para revertirlo limpio si al cliente no le gusta el resultado en vivo (ya paso).
- Layout de cards: jerarquia por estilo (badge/borde), no por tamaño desproporcionado, salvo pedido explicito del cliente.
- **Baseline de movimiento por default en todo sitio nuevo** (validado en olvidatasoft-new, detalle y trampas en el rol): fondo de red de nodos derivado del isotipo del cliente (leer su SVG antes de diseñarlo — forma, color unico y jerarquia de tamaños salen de ahi), titulos con efecto maquina de escribir **en todos los `<h1>` y `.section-heading`** con su par accesible, y cascada escalonada en toda grilla de cards. Se aplica sin esperar a que lo pidan; lo que si se pregunta es si el cliente quiere pagar el peso de la variante WebGL (~180 KB gz) o alcanza con Canvas2D.
- **El movimiento va sobrio**: si un efecto compite con la legibilidad del texto o satura la pantalla, esta mal calibrado. Ante la duda, la version mas contenida — es mas facil que pidan subirlo que defender algo recargado.
